...
func (t Training) distance() float64 {
	// код ниже это формула расчета и возврат дистанции, которую преодолел пользователь
	distanceTraining := float64(t.Action) * t.LenStep / MInKm
	return distanceTraining
}

// meanSpeed возвращает среднюю скорость бега или ходьбы.
func (t Training) meanSpeed() float64 {
	// код ниже это проверка на 0, формула средней скорости бега или ходьбы и ее возврат
	if t.Duration == 0 {
		return 0
	}
	meanSpeedTraining := (float64(t.Action) * t.LenStep / MInKm) / float64(t.Duration)
...
func (t Training) Calories() float64 {
	// код ниже просто возвращает 0, ниже этот метод будет переопределен 3 раза
	return 0
}
...
type CaloriesCalculator interface {
	// добавляем необходимые методы в интерфейс
	Calories() float64
...
func (r Running) Calories() float64 {
	// код ниже это формула расчета количества потраченных килокалория при беге и возрат этого
	burnedCaloriesWhileRunning := (CaloriesMeanSpeedMultiplier*r.meanSpeed() + CaloriesMeanSpeedShift) * r.Weight / MInKm * r.Duration.Hours() * MinInHours
	return burnedCaloriesWhileRunning
}

// TrainingInfo возвращает структуру InfoMessage с информацией о проведенной тренировке.
// Это переопределенный метод TrainingInfo() из Training.
func (r Running) TrainingInfo() InfoMessage {
	// код ниже это вызов к структуре Running, в ней к структуре Training - как к элементу структуры Running, и уже у Training вызывается TrainingInfo()
...
type Walking struct {
	// добавляем необходимые поля в структуру
	Training
...
func (w Walking) Calories() float64 {
	// код ниже это формула расчета количества потраченных килокалория при ходьбе и возрат этого
	burnedCaloriesWhileWalking := ((CaloriesWeightMultiplier*w.Weight + (math.Pow(w.meanSpeed(), 2)/(w.Height/CmInM))*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours)
	return burnedCaloriesWhileWalking
}

// TrainingInfo возвращает структуру InfoMessage с информацией о проведенной тренировке.
// Это переопределенный метод TrainingInfo() из Training.
func (w Walking) TrainingInfo() InfoMessage {
	// код ниже это вызов к структуре Walking, в ней к структуре Training - как к элементу структуры Walking, и уже у Training вызывается TrainingInfo()
...
func (s Swimming) meanSpeed() float64 {
	// код ниже это проверка на 0, формула средней скорости плавания и ее возврат
	if s.Duration == 0 {
		return 0
	}
	meanSpeedForSwimming := (float64(s.LengthPool) * float64(s.CountPool) / MInKm / float64(s.Duration))
	return meanSpeedForSwimming
...
func (s Swimming) Calories() float64 {
	// код ниже это формула расчета количества потраченных килокалория при плавании и возрат этого
	burnedCaloriesWhileSwimming := (s.meanSpeed() + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * s.Weight * s.Duration.Hours() * MinInHours
	return burnedCaloriesWhileSwimming
}

// TrainingInfo returns info about swimming training.
// Это переопределенный метод TrainingInfo() из Training.
func (s Swimming) TrainingInfo() InfoMessage {
	// код ниже это вызов к структуре Swimming, в ней к структуре Training - как к элементу структуры Swimming, и уже у Training вызывается TrainingInfo()
	return s.Training.TrainingInfo()
}

// ReadData возвращает информацию о проведенной тренировке.
func ReadData(training CaloriesCalculator) string {
	// получаю количество затраченных калорий
